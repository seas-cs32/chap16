...
with em dashes rather than the existing commas.""")
    ...
    for i, s in enumerate(sentences):
        ...
        a: int = get_phrase_index(phrases)
        if a == 0:
        ...
        new_s: str = s_prefix + '--' + phrases[a].split() + '--' + s_suffix.split()
...
def get_phrase_index(phrases) -> int:
    ...
    while True:
        try:
            ...
            if ans == 0:
                ...
                phrase_index: int = ans - 1  # Turn ans into an index
                return phrase_index
        except ValueError:
